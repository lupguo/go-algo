...
func TestSelect(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if gotSortedList := Select(tt.args.A, len(tt.args.A)); !reflect.DeepEqual(gotSortedList, tt.wantSortedList) {
				t.Errorf("Select() = %v, want %v", gotSortedList, tt.wantSortedList)
			}
		})
	}
}